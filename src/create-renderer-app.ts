/**
 * Renderer App — composition root that wires together all modules.
 * @module create-renderer-app
 */

import { createBackend } from './backend';
import { createPaneManager } from './manager/create-pane-manager';
import { createFocusController } from './manager/create-focus-controller';
import { createPaneStateCompat } from './compat/pane-state-compat';
import { createBridgeCompat } from './compat/bridge-compat';
import { createBridge, clearLayoutWindowBinding } from './compat/bridge-compat';
import { createPaneRenderer, getTextColorForBackground } from './compat/pane-renderer-compat';
...
export interface RendererAppDeps {
  ...
  addPaneButtonEl: HTMLElement;
  addProfileButtonEl: HTMLElement;
...
export interface RendererApp {
  ...
  getKeyboardActions(): ReturnType<typeof createActions>;
  getDispatchKeydown(): (event: KeyboardEvent) => void;
  getCurrentMode(): string;
  paneState: ReturnType<typeof createPaneStateCompat>;
  bridge: ReturnType<typeof createBridgeCompat>;
...
export function createRendererApp(deps: RendererAppDeps): RendererApp {
  const {
    stageEl, tabsListEl, statusLabelEl, statusHintEl,
    addPaneButtonEl, addProfileButtonEl, layoutsButtonEl,
    settingsButtonEl, fullscreenButtonEl, settingsPanelEl,
    shellProfilesSettingsBtn, layoutsSettingsBtn,
  ...
  // ---- New modules (PaneManager, FocusController, Backend) ----

  const tauriGlobal = (window as any).__TAURI__ ?? (window as any).vibe99 ?? null;
  const rawBridge = createBridge(tauriGlobal, null);

  const backend = createBackend(tauriGlobal && typeof tauriGlobal === 'object' && 'core' in tauriGlobal ? tauriGlobal : null as any);
  ...
  const focusController = createFocusController(
    {
      getFocusedPaneId: () => paneManager.getFocusedPaneId(),
      getPanes: () => paneManager.getPanes(),
      getPaneIndex: (id: string) => paneManager.getPaneIndex(id),
      focusPane: (id: string, opts?: { focusTerminal?: boolean }) => paneManager.focusPane(id, opts),
    },
    {
      onStateChange: () => {},
      onModeChange: (mode: string) => { currentMode = mode; document.body.classList.toggle('is-navigation-mode', mode === 'nav'); },
    },
  );

  const paneState = createPaneStateCompat({
    paneManager,
    focusController,
    defaultCwd: rawBridge.defaultCwd,
    defaultTabTitle: rawBridge.defaultTabTitle,
    getAccentPalette: () => [...ColorsRegistry.ACCENT_PALETTE],
    onStateChange: () => {},
  });

  const bridge = createBridgeCompat({
    backend,
    tauri: tauriGlobal,
    currentWindowLabel: rawBridge.currentWindowLabel,
    defaultCwd: rawBridge.defaultCwd,
    defaultTabTitle: rawBridge.defaultTabTitle,
    layouts: rawBridge.layouts,
    onMenuAction: rawBridge.onMenuAction,
    onLayoutFocusNotice: rawBridge.onLayoutFocusNotice,
  });
  ...
  // ---- Event listeners (returned for renderer.ts to bind) ----
  ...
  const keyboardActions = createActions({
    ...
    enterNavigationMode: () => {
      ...
      const sourceId = paneState.getFocusedPaneId();
      focusController.enterNavigationMode();
      paneOps?.blurFocusedTerminal();
      render();
    },
    cycleToRecentPane: (...args) => paneOps?.cycleToRecentPane(...args),
    cycleToNextLitPane: (...args) => paneOps?.cycleToNextLitPane(),
    navigateLeft: (...args) => paneOps?.navigateLeft(),
    navigateRight: (...args) => paneOps?.navigateRight(),
    ...
    cancelNavigationMode: () => {
      ...
      if (sourceId) {
        paneOps?.focusPane(sourceId, { focusTerminal: true });
      } else {
        setMode('terminal');
        render();
      }
  ...
  return {
    ...
    getDispatchKeydown: () => dispatchKeydown,
    getCurrentMode: () => currentMode,
    paneState,
    bridge,
    paneRenderer,
    focusPane: (...args) => paneOps?.focusPane(...args),
    closeTopModal: () => modalStack.closeTop(),
    isModalStackEmpty: () => modalStack.isEmpty(),
  };
}
