...
// Capture-phase ESC listener to try preventing the browser/webview from
// exiting fullscreen. Some platforms (WebKit) ignore preventDefault() in
// bubble phase, so we also block it as early as possible.
window.addEventListener('keydown', (event) => {
  if (event.key === 'Escape') {
    event.preventDefault();
