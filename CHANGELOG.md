...
- **Color + Shell capabilities (VIB-215):** Created `src/pane/capabilities/color-capability.ts` (61 lines) with `createColorBehavior(deps)` factory returning `PaneCapability<ColorCapabilityApi>` with `getAccent()`, `setCustomColor(color)`, `clearCustomColor()`. Coordinates with terminal-capability (`setTheme`) and dom-capability (`setAccent`) for color updates. Created `src/pane/capabilities/shell-capability.ts` (63 lines) with `createShellBehavior(deps)` factory returning `PaneCapability<ShellCapabilityApi>` with `changeProfile(profileId)` that coordinates pty-capability (`beginShellChange`, `endShellChange`, `create`) and terminal-capability (`clear`). No `any` types.
- **PTY capability extraction (VIB-213):** Created `src/pane/capabilities/pty-capability.ts` (118 lines) with `createPtyBehavior(deps)` factory returning a `PaneCapability<PtyCapabilityApi>` with `name: 'pty'`. Extracted PTY session management logic from `pane-renderer.ts` into a dedicated capability module. API includes: `sessionReady` (getter), `create(cols, rows)` (via `backend.terminal.create`), `write(data)`, `resize(cols, rows)`, `destroy()`, shell change state management (`beginShellChange()`, `endShellChange()`, `isShellChanging`, `recentShellChange`). Handles PTY data callback (`backend.terminal.onData`) and exit callback (`backend.terminal.onExit`). No `any` types. This is Phase 2 step 4 for the pane-capability refactor.
...
- **Backend layer extraction (VIB-209):** Created `src/backend.ts` (~215 lines) that exports `createBackend(tauri)` function returning domain-grouped interfaces: `terminal` (create, write, resize, destroy, onData, onExit), `clipboard` (read, write, snapshot), `settings` (load, save), `shell` (list, add, remove, setDefault, detect), `window` (close, openUrl, showMenu), plus `platform` and `cwdReady`. Types re-exported from `bridge.ts` for consistency. Old `bridge.ts` retained unchanged — no modifications to existing call sites (`renderer.ts`, `pane-renderer.ts`, etc.). This is Phase 2 step 1 for the pane-capability refactor.
- **DOM capability extraction (VIB-211):** Created `src/pane/capabilities/dom-capability.ts` (98 lines) with `createDomBehavior(deps)` factory returning a `PaneCapability<DomCapabilityApi>` with `name: 'dom'`. Extracted DOM creation logic from `pane-renderer.ts` into `createDomApi(deps, paneId, accent)` helper. DOM structure: `root(article.pane) > shell > body > surface > terminalHost`. API includes `root`, `terminalHost`, `mount(container)`, `unmount()`, `setLayout({ left, height, zIndex })`, `setFocused(isFocused, isNavTarget)`, `setAccent(color)`, `dispose()`. Integrates breathing mask (`paneAlert.attach()`) and registers click focus / right-click context menu events. Updated `pane-renderer.ts` to use `createDomApi` for DOM creation, and updated `renderer.ts` `onTerminalContextMenu` to look up the node via `paneRenderer.getNode(paneId)`.
