...
- **E2E Docker image bloat reduction (VIB-276):** Merged cleanup steps into the same RUN layers to reduce image size: clean npm cache after each `npm install`, remove Cargo registry cache and target incremental/build/fingerprint intermediates after `tauri:build-dev`. Added `.git/`, `.gitattributes`, `.github/` to `.dockerignore` to prevent unnecessary files from entering the build context. Target: < 3 GB.
