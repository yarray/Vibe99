...
- **PTY capability extraction (VIB-175):** Created `src/pane/capabilities/pty-capability.ts` (~131 lines). Factory `createPtyBehavior(deps)` returns `{ name: 'pty', open(ctx), close() }`. `open(ctx)` returns `PtyCapability` API with: `sessionReady` getter, `create(cols, rows)` creates backend PTY session, `write(data)` sends input to PTY, `resize(cols, rows)` resizes PTY, `destroy()` destroys PTY session, `beginShellChange()`/`endShellChange()`/`isShellChanging`/`recentShellChange` for shell-switch state tracking, `close()` unsubscribes backend listeners and destroys PTY. Registers `backend.terminal.onData` → `terminal.write` and `backend.terminal.onExit` → `deps.onExit`. Also registers `terminal.onData` → `backend.terminal.write` when session is ready. Uses `Backend` grouped API. Error handling matches existing behavior (writes failure message to terminal).
